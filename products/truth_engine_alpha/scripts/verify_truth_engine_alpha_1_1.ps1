param(
    [string]$BaselineVerificationDir = "results/verification/product_baseline_20260804_155015"
)
...
$verificationRoot = Join-Path $root "results/verification"
$timestamp = Get-Date -Format "yyyyMMdd_HHmmss"
$outDir = Join-Path $verificationRoot "truth_engine_alpha_1_1_verify_$timestamp"
New-Item -ItemType Directory -Path $outDir -Force | Out-Null

$baselineDir = Join-Path $root $BaselineVerificationDir
if (-not (Test-Path $baselineDir)) {
    throw "Baseline verification directory not found: $baselineDir"
}

$env:PYTHONPATH = Join-Path $root "src"

$testOut = Join-Path $outDir "full_test_output.txt"
python -m pytest tests -q > $testOut 2>&1
$testExit = $LASTEXITCODE
"EXIT_CODE=$testExit" | Set-Content -Path (Join-Path $outDir "full_test_exit_code.txt") -Encoding UTF8
if ($testExit -ne 0) {
    Write-Output "VERIFICATION_DIR=$outDir"
    throw "Test suite failed. See $testOut"
}
...
$pyPath = Join-Path $outDir "_run_release_gates.py"
Set-Content -Path $pyPath -Value $pyScript -Encoding UTF8
python $pyPath $root $baselineDir $outDir > (Join-Path $outDir "release_gate_python_output.txt") 2>&1
$gateExit = $LASTEXITCODE
"EXIT_CODE=$gateExit" | Set-Content -Path (Join-Path $outDir "release_gate_exit_code.txt") -Encoding UTF8
if ($gateExit -ne 0) {
    Write-Output "VERIFICATION_DIR=$outDir"
    throw "Release gates failed. See $outDir"
}

Write-Output "VERIFICATION_DIR=$outDir"
Get-Content (Join-Path $outDir "release_gate_summary.txt")
