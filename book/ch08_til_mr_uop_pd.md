...
The **Unified Optimization Principle (UOP)** says reasoning optimizes a *joint* quantity — truth balanced against existence (the GILE pillar against the HEM pillar of Chapter 6) — and that this joint optimum sits at an **interior** point, not at the ceiling. In the framework's scaling the optimum lands at the **Radiant Cap, G\* = √(1 − e⁻²) ≈ 0.93**, with a *penalty for pushing above it* **(framework-internal)**. The *value* is fixed by tying truth to existence — the cap is the square root of the Existence floor `1 − e⁻²`, so `Existence = G*²` exactly (Chapter 5) — and its *interior* location reflects that real value needs both *coherence* and *reserved freedom*, each of which is killed at one extreme, so the maximum falls in the middle, leaving roughly a 7% "Freedom Floor" permanently open.
