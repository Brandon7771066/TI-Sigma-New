...
from eeg_bci_system import (
    EEGSignalProcessor, 
    MotorImageryClassifier, 
    MotorImageryDirection,
    TIMetrics,
    EEGConfig
)
from muse_streaming import (
    MuseEEGStream,
    HRVSimulator,
    IntegratedBCIStream,
...
def initialize_session_state():
    ...
    if 'classifier' not in st.session_state:
        st.session_state.classifier = MotorImageryClassifier(st.session_state.processor)
...
def update_player_paddle_eeg(game: PongGameState, 
    ...
    if muse_stream:
        ...
        if sample:
            samples = {ch: sample[ch] for ch in ['TP9', 'AF7', 'AF8', 'TP10'] if ch in sample}
...
def main():
    st.set_page_config(
        page_title="EEG Pong - Brain-Controlled Gaming",
        page_icon="🧠",
        layout="wide"
    )
    ...
    with col1:
        ...
        if game.game_over:
            if game.winner == "PLAYER (YOU!)":
                st.success(f"🎉 VICTORY! You won {game.player_score}-{game.ai_score}!")
            else:
                st.error(f"Game Over! AI wins {game.ai_score}-{game.player_score}")
    ...
    with col2:
        ...
        if game.move_log:
            ...
            lcc_moves = [m for m in game.move_log if m.get('source') == 'lcc_hypercomputer']
            keyboard_moves = [m for m in game.move_log if m.get('source') == 'keyboard']
            ...
            if consciousness_moves:
                avg_lexis = np.mean([m.get('lexis', m.get('Lexis', 0)) for m in consciousness_moves])
