...
- **DEPRECATE** — descriptor labels a non-existent / over-extended region; recommend removal

Brandon's verdict per row controls; the entire table is a proposal until Brandon-ratified.

---

## 1. Label-by-label audit
...
| 6 | **Transcendent Annulus** | (complex plane) the annulus { z ∈ ℂ : φ < \|z\| < e } — between the φ and e bounding circles | **RENAME (proposal)** | **φ-to-e Annulus** OR **Transcendent Ring** | "Annulus" is mathematically precise but heavy; "Ring" is more readable. Brandon ratification controls. |
| 7 | **Standard Zone** | (real axis) interval (1, φ) — between unit boundary and golden-ratio threshold | **KEEP** | — | Established usage; no ambiguity. |
...
## 4. Implementation if Brandon ratifies

If Brandon ratifies this audit (whole or in part), implementation steps:

1. Update `TI_SIGMA_ABBREVIATIONS_CONCEPTS_THEORIES_INDEX_2026-05-07.md` §2.2 with ratified renames + ratified deprecations
2. Search-and-replace across `papers/` for all RENAME items — preserve original strings as parenthetical "(formerly: …)" for one-version transition period
3. Move DEPRECATED items #8, #9 to TIC vocabulary section (if applicable), or delete entirely
4. Update `PD_SPECTRUM_DT_IMAGINARY_AXIS_EMERICK_CROSSOVER` body to use ratified names
5. Update `PD_COMPLEX_PLANE_RECANONIZATION_PASS_8` §1 + §2 to use ratified names
6. Update book chapters that cite these labels (none currently — Pass 8 RECANONIZATION did not propagate labels into book body)

Estimated implementation cost: ~30 minutes once Brandon ratification is in.

---

## 5. Brandon ratification template

To ratify, Brandon may simply respond with one of:
- **"Ratify all proposals"** — adopt every KEEP/RENAME/DEPRECATE as listed
- **"Ratify with exceptions: [list]"** — adopt all except specified rows
- **"Ratify only: [list]"** — adopt only specified rows
- **Per-row override:** "Row #N: [KEEP / RENAME to "X" / DEPRECATE]"

Or supply Brandon's own preferred names where the proposed alternatives don't resonate.

---

*End of Pass 8.1 PD Label Audit.*
