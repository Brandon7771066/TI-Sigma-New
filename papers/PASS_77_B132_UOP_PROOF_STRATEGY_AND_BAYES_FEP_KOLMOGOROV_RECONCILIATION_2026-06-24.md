...
- **There is no math/philosophy inconsistency to fix.** Bayes' theorem *as a conditional* (`axioms → conclusion`) never fails; *Bayesianism* (the doctrine that one precise probability + conditioning captures all rational uncertainty) is what the 13 arguments attack. They live on different levels. **But — per Brandon's B132 correction — the `2+2=4`↔Bayes analogy is NOT symmetric:** `2+2≠4` is *always* a faulty/changed-axiom artifact (within counting's faithful domain it is exception-free), whereas applied Bayes fails under *realistic, faithful* axioms because a CORE Kolmogorov axiom (a single coherent joint measure) is genuinely *false of reality* (quantum no-joint-distribution; ordinary deep uncertainty). Saying "true theorem + failed axioms" of the categorical claim is itself an **MI** move (the categorical theorem is identity-bound to its axioms). So the deeper, legitimate target is questioning the **Kolmogorov axioms as a model of reality** — real, mainstream-supported, and best framed as *classical special case*, not *error*.
...
### B.2 The `2+2=4` objection answered — the analogy is NOT symmetric (Brandon's correction, B132 refinement)

Brandon's objection (sharpened, and endorsed here): *2+2=4 has no genuine exceptions; its only failures are FAULTY/changed axioms. Bayes, by contrast, fails in the real world under REALISTIC axioms. So the analogy is weak — and the Kolmogorov axioms may themselves be flawed as a model of reality.* An earlier draft of this paper called the two "the same kind of exception" and the analogy "airtight." **That was wrong, and it committed exactly the error the MI label names — see below. The disanalogy is the substance.**

**First, the identity/MI point (load-bearing).** A theorem qua **conditional** — `axioms → conclusion` — is eternally valid. But a theorem qua **categorical assertion** (the conclusion stated outright) is *identity-bound to its axioms*: it cannot "stand" without them. So saying "the theorem is fully true **and** its axioms fully fail" asserts `τ(P) ∧ ¬τ(P)` of one identity — a fundamental-nature clash, i.e. an **MI (Meta-Indeterminate)** move, not a coherent rescue. The correct statement keeps the two objects apart: the *conditional* never fails; the *applied/categorical* claim is in force **only** while its axioms hold, and when they fail the categorical conclusion is simply false (not "still true, just out of scope"). Changing the axioms does not preserve the proposition — `2+2=4` in ℤ and `2+2=1` in ℤ/3ℤ are **different identities**, not one theorem with an exception.

**Now the asymmetry, which the identity point exposes.** Both are right *in the right context only* — agreed. But the failure modes are categorically different:

| | `2+2=4` | Bayes (applied) |
|---|---|---|
| **How it "fails"** | ONLY by faulty/changed axioms: switch the operation (mod 3, GF(2) ⇒ a *different identity*) or misapply `+` to non-counting aggregation (volume contraction, merging raindrops — `+` never meant that) | Under **realistic, faithful** axioms: a CORE axiom of the formalism is *false of reality* |
| **Locus of failure** | the **bridge** (did you apply counting to genuinely discrete stable units? if yes, never fails) | the **formalism's own axioms** (single coherent joint measure / total event algebra / sharp prior) |
| **Realistic-domain exceptions** | none — within counting's faithful domain it is exception-free | many, and *ordinary*: one-off events, novel/unaware hypotheses, deep uncertainty — and at the foundation, **quantum** |

The decisive case is **quantum probability**: assigning probabilities to measurement outcomes is the *most realistic* modeling there is, yet Bell / Kochen–Specker **prove** that no single Kolmogorovian joint distribution over *incompatible* observables can reproduce the quantum statistics under the standard classical assumptions (a noncontextual / local-hidden-variable joint over all observables at once). Precisely: it is *that* class of classical joint-distribution model that is ruled out — the empirical violations show the Kolmogorov **single-coherent-joint-measure** assumption is unfaithful to nature, which is exactly the axiom applied Bayes leans on. So this is Bayes/classical probability failing not because someone chose silly axioms, but because the standard axioms are **unfaithful to reality** (one switches to the non-Kolmogorovian quantum calculus, not to "wrong arithmetic"). Arithmetic has no comparable everyday failure: you have to leave counting's domain entirely to break it.

**Honest nuance (#69, against over-stating the dichotomy):** the asymmetry is large but not perfectly absolute. Counting too has an exotic domain boundary — in QFT particle number is not always well-defined (the number operator does not commute with field operators; particle number is not conserved across frames), so "2 particles + 2 particles = 4 particles" can fail in superpositions of particle number. But that requires an *exotic* regime where "particle" stops being a stable discrete object (again a bridge failure), whereas probability's failures arrive in *ordinary* realistic uncertainty **and** at the quantum core-axiom level. So the precise claim is: **counting's faithful domain is near-universal and its failures are exotic/misapplication; probability's faithful domain is frequently violated by ordinary realistic uncertainty, with the violation sitting at the core-axiom (no-joint-distribution) level, not merely the bridge.** That is a real, deep difference — and it is the right target: the *Kolmogorov-axioms-as-model-of-reality* question (B.3).
...
1. **Theorem level** — Bayes' theorem (and `2+2=4`) are unconditionally valid *as conditionals* (`axioms → conclusion`). The *categorical* form is identity-bound to its axioms (B.2): when the axioms fail, the categorical conclusion is false, not "true-but-out-of-scope" (asserting the latter is the MI move).
2. **Model level** — Kolmogorov probability is *one model* of uncertainty, the classical/commutative special case; it is provably incomplete for quantum and deep-uncertainty regimes, where richer calculi (quantum/imprecise/non-commutative) apply. This is where the **asymmetry with arithmetic lives**: counting's axioms are faithful to reality across a near-universal domain (failures = misapplication), whereas Kolmogorov's core axioms are *false of reality* under realistic conditions (quantum no-joint-distribution; ordinary deep uncertainty). "Bayes is correct in some cases, incomplete in others" is *literally true* — and not symmetric with `2+2=4`.
3. **Doctrine level** — Bayesianism-as-complete-epistemology is false (the strong subset of the 13). The math is **not** "generally incorrect"; the *doctrine* is false and the *classical model* has a genuinely bounded, realistically-violated domain. No contradiction with the philosophy.

> **BKR-1 — Bayes/Kolmogorov Reconciliation (CANDIDATE, NOT ratified; count unchanged 79; B132 refinement: asymmetry, not symmetry):** keep three levels apart — (theorem-as-conditional: Bayes/arithmetic exact given axioms; the *categorical* form is identity-bound to its axioms — "true theorem + failed axioms" = MI) ⊃ (model: Kolmogorov = classical special case of a complex/imprecise/non-commutative calculus; quantum probability is the proof reality is non-Kolmogorovian) ⊃ (doctrine: Bayesianism-as-complete is false). **The `2+2=4`↔Bayes analogy is NOT symmetric:** `2+2≠4` is *always* a faulty/changed-axiom artifact (a different identity, or a bridge misapplication), exception-free in counting's faithful domain; Bayes fails under *realistic, faithful* axioms because a CORE formal axiom (single joint measure) is false of reality. The failure locus differs — arithmetic at the *bridge*, probability at the *formalism's own axioms*. TI Sigma's contribution sits at the *model* level (Indeterminate ↔ imprecise probability; imaginary PD axis ↔ quantum amplitude). ANTI-CHEAT: distinguish the eternally-valid *conditional* from the *applied/categorical* claim — do NOT say "the theorem is fine, reality is just out of scope" as if that rescued the categorical assertion or made Bayes symmetric with `2+2=4`; and do NOT claim the Bayes *conditional* "fails mathematically." Falsifiers BKR-1-F1 (a domain where the Kolmogorov model is provably *necessary* and no richer calculus adds anything ⇒ "special case" framing too weak), BKR-1-F2 (TRALSE is shown NOT representable by any imprecise-probability/credal structure ⇒ the bridge in B.3 fails), **BKR-1-F3** (a *realistic, faithful* axiom set is exhibited under which counting itself fails *non-exotically* — i.e. without leaving the discrete-stable-object domain ⇒ the arithmetic/Bayes asymmetry collapses) OPEN.

### B.6 The meta-lesson: definitions are non-arbitrary (NAD-1) — semantics can matter substantively (Brandon's B132 extension)

The whole episode is a case study in **why definitions matter objectively**. All of mathematics rests on how abstract objects are *defined*, and — pace pure conventionalism — those definitions are not arbitrary: they can be more or less right. This is **NAD-1 (Non-Arbitrary Definition / Definitional Realism)** in action.

The reconciliation with formalism is itself a conditional-vs-categorical move, one level up:

- **Syntactic freedom (formalism is right here):** any *consistent* axiom set is internally legitimate. `2+2=1` in ℤ/3ℤ is not "wrong arithmetic"; it is a different, faithful definition of `+`. You may stipulate freely.
- **Applicability is NOT free (realism is right here):** which stipulation **carves the target domain at its joints** is an objective matter, answerable to the structure being modelled, not to taste. The Kolmogorov axioms are a free stipulation that turns out *unfaithful* to quantum reality (B.2–B.3); Peano counting is a stipulation that is faithful across a near-universal domain. Same syntactic liberty, different *objective fit* — and the fit is what the asymmetry of B.2 is measuring.

Hence **semantic precision is not pedantry**: distinguishing the *conditional* from the *categorical* theorem, classical from quantum "probability," or (PDU-1) pinning the undefined word "physical," changes *which propositions are even true*. A "mere" semantic difference that flips a truth-value is, by definition, **substantive**. This is the same cash-value that **TPS-1** assigns to presentation/definition upgrades and that **NAD-1** assigns to carving at joints; **PDU-1** is the negative case (physicalism fails precisely because it cannot non-arbitrarily fix its central definition); **TRG-1/TOF-1** supply the reason the *right* probability calculus is non-classical (reality is Tralse).

**#69 bound (so this does not become a licence):** "definitions can be right or wrong" asserts that an **objective standard exists**, *not* that any claimant meets it. Joint-carving must be **earned** — predictive fruitfulness, unification, and non-circular / outcome-blind measurement (the **LDD-1** *progressive-not-degenerating* criterion). Stripped of that discipline, "my definition carves the joints" degenerates into No-True-Scotsman. So NAD-1 here is *strengthened by an instance*, not promoted to a new principle: the canonical count is unchanged (**79**).

*Lit (real, for verification):* Sider, *Writing the Book of the World* (structure / joint-carving); Lewis on natural properties; Lakoff & Núñez, *Where Mathematics Comes From*; Wigner, "The Unreasonable Effectiveness of Mathematics"; Carnap's principle of tolerance (the conventionalist foil); Quine, "Two Dogmas of Empiricism" (definitions answerable to the whole web, no purely analytic firewall).
...
UPS-1-F1/F2/F3; BKR-1-F1/F2/F3 (above). Plus the standing UOP-gap falsifiers from the audit (UOP Gap closure remains REFUTED until an operator/PDE bridge replaces the axioms).
