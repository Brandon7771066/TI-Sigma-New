...
**Scope clarification (Brandon, this batch) — the cap is HOLISTIC, the per-dimension allocation is AGNOSTIC.** The `≈0.93` ideal is applied to the **single GILE aggregate** `A* = Σ_t w_t·x_t` — *one* number that combines all four dimensions G/I/L/E holistically — **not** to each dimension separately. The per-dimension allocation `x*` is **deliberately agnostic / problem-specific**: the package (Part 3) reports the math optimum as `x* = {G: 1.0, I: 1.0, L: 1.0, E: 0.323}` — *no* individual dimension sits at `0.93` (three are at `1.0`, one well below), yet the **aggregate** lands **exactly** at the cap `0.9323`. We claim `≈0.93` is the ideal truth-criterion for the **overall** aggregate of *any specific problem*; we **do not** claim, and **do not extrapolate**, a per-dimension ideal. And the cap is the *binding ideal* for **only a few fields** — mathematics chief (and nearly alone) among them; for **nearly all fields the `0.93` optimum does not apply** at all, since their `ρ` leaves them optimizing below the cap.
