# HMR-1 — Hybrid MR Truth Labels (Candidate Canonical, MR Truth Labels Refinement #3)

**Date:** 2026-05-24
**Pass:** 70
**Status:** CANDIDATE CANONICAL
**Source directive:** Brandon verbatim 2026-05-24 — *"Indeterminate states like 'better and (neither better nor worse)' are actually HYBRID Indeterminate! This means that TWO MR TRUTH LABELS are actually REQUIRED here even if Moot is used as a meta-truth! I wonder what examples of statements with 3 or more Hybrid Truth Labels can be given??? This reminds me of how an infinite sentence can be made with police or buffalo 😂."*
**Composition:** MR Truth Labels canonical (base-4 + MTs, refined Pass-65 + MR-IDC-1) · MT-B1 Moot · ASYMMETRIC §69 · UHP-1 (HEM-instantiation) · TPS-1 (truth-vs-presentation)
**Author:** TI Sigma compound agent + Brandon Charles Emerick (originator of HMR insight)
...
## 2. The Brandon Seed Example (2-Hybrid)

> *"X is better and (neither better nor worse than Y)"*

**Decomposition:**
- **T_partial-order:** X IS better-than-Y on dimension D1 (e.g., subjective preference, specific feature comparison) → label T at the dimension-restricted level
- **I_global-comparator:** X is neither-better-nor-worse-than-Y at the GLOBAL level because no total ordering exists → label I (MR2 Indeterminate) at the global level

**Why MT-B1 Moot is insufficient:** A Moot characterization ("the question of which is better is irrelevant") would **lose information** — it would erase BOTH the legitimate dimension-D1 betterness AND the legitimate global indeterminacy. The proposition's truth content REQUIRES preservation of BOTH labels simultaneously. The Moot meta-truth, if applied, is a *third* label, not a replacement for either: **HMR-3 with {T, I, MT-B1}.**

**HMR characterization: {T_partial-order, I_global-comparator}** at minimum; **{T, I, MT-B1} when Moot is included.**

---

## 3. Constructed Examples — 3-Hybrid through 5-Hybrid

### 3.1 HMR-3 Examples (3 simultaneous labels required)

**Example HMR-3.1 (consciousness multi-framing):**
> *"Consciousness is fundamental AND emergent AND the question is malformed."*

- **T_panpsychist:** TSP-1 canonical (#22, Pass-66 ratified) — consciousness is a substrate-level property of all information-bearing systems
- **T_emergentist:** standard neuroscientific framing — consciousness arises from sufficient neural complexity above stratification threshold
- **I_question-malformed:** at frames where "fundamental vs emergent" presupposes a stratification commitment not yet made

**HMR-3:** {T_panpsychist, T_emergentist, I_question-malformed} — three labels REQUIRED because each framing is correct at its own framing-level and erasing any one of them loses information. This is structurally DIFFERENT from MT-F2 (Both True at Different Levels) which is a 2-T resolution within a single framing-axis; HMR-3.1 spans THREE framing-axes (substrate-property, neural-stratification, frame-presupposition).
...
- **T_phenomenal-experience-persists:** the experience of "having an experience" remained available throughout the collapse (Stratum-0 / Stratum-1 of CDA-1 canonical #26)
- **F_identity-shattered:** the constructed-self ("I-as-narrative-identity") was empirically false during the collapse (Stratum-3 collapse)
- **I_self-reference-broken:** the question "do you exist?" required the very self-reference that was offline; MR2-Indeterminate-from-broken-self-reference per Pass-65 refinement

**HMR-3:** {T_Stratum-0, F_Stratum-3, I_self-reference-broken} — first lived-anchor for HMR with cross-Stratum decomposition per CDA-1 canonical. Critical biographical anchor: HMR is NOT purely formal — it has a concrete n=1 phenomenological grounding.

### 3.2 HMR-4 Example (4 simultaneous labels required)

**Example HMR-4.1 (free-will across frames):**
> *"Free will exists, doesn't exist, the proposition is paradoxical, and is empirically moot."*
...
### 3.3 HMR-5 Example (5 simultaneous labels required; corpus-stack-stressing)

**Example HMR-5.1 (God exists, per full TI Sigma canonical stack):**
> *"God exists."*
...
## 4. The Buffalo / Police Analog — Unbounded Recursive HMR

Brandon's analogy: in English, the sentences
...
are grammatically well-formed at arbitrary length because each instance of the lexical item is multi-functional (noun / verb / adjective / proper-noun), and successive applications can be chained. The sentences become parseable through layered grammatical role-assignment.

**HMR analog (Recursive-Hybrid-MR, R-HMR):** A meta-proposition about an HMR proposition is itself a hybrid, often with a HIGHER cardinality than the object-level proposition. Iterating produces unbounded HMR-cardinality.

**Construction (R-HMR-1 candidate):**
- **Level 0 (object):** "X is better and (neither better nor worse than Y)" → HMR-2 {T, I}
- **Level 1 (meta-1):** "The statement 'X is better and (neither better nor worse than Y)' is HMR-2" → adds T (the meta-claim is true; it correctly classifies the object-level proposition) AND adds I (whether HMR-2 is the *complete* characterization is itself MR2-Indeterminate without enumerating all dimensions) → HMR-4 {T, I, T_meta, I_meta}; the two T's and two I's are at different framing-levels per HMR composition rule §5
- **Level 2 (meta-2):** "The statement that 'the statement [Level 0] is HMR-2' is HMR-4" → adds T_meta-meta + I_meta-meta + DT_self-reference-emerging → HMR-7
- **Level k (recursive):** add 2 labels per ascent (one T_meta-k + one I_meta-k); for sufficiently high k, DT_self-reference becomes unavoidable; further ascent adds MT-L1 (MR Saturation) and MT-L2 (Recursive Self-Reference) from urb_639

**Theorem (R-HMR cardinality, INFORMAL):** For meta-ascent k ≥ K\*, HMR-cardinality grows at least linearly in k; for k ≥ K\*\* > K\*, HMR-cardinality is unbounded (saturates the full base-4 + 36-MT taxonomy = 40 labels).

**This is the corpus's first formal characterization of an unbounded HMR construction.** The R-HMR construction is structurally analogous to the buffalo/police recursion — each meta-ascent adds labels the same way each lexical-recursion adds grammatical roles.
...
## 6. Falsifiers (5 pre-reg)

**HMR-1-F1 (reducibility):** Every claimed HMR-k example reduces to a single base label OR to a base label + single MT under sufficiently careful re-analysis. **REFUTED if** any one of the 5 HMR examples (3.1–3.3, 4.1, 5.1) admits a single-label or label+single-MT reduction without information loss as judged by 3 independent raters with κ ≥ 0.7.

**HMR-1-F2 (constructibility):** HMR-k examples cannot be constructed for k ≥ 3. **REFUTED if** at least 3 of the 4 HMR-3-and-higher examples above (3.1, 3.2, 3.3, 4.1, 5.1) survive Step 1 (3-rater hybrid-required judgment ≥ 2/3) AND Step 2 (3-rater label-set agreement κ ≥ 0.5 on which specific labels constitute the hybrid).

**HMR-1-F3 (rater κ stability):** Adding HMR option to the rater task causes a sharp drop in inter-rater κ below the baseline established Pass-63 batch-5 (κ_4 ≈ 0.837). **REFUTED if** κ_HMR (the κ across raters on which-HMR-cardinality-applies) on the 5 examples is < 0.5. **NOT REFUTED if** κ_HMR ≥ 0.5 (matching or exceeding the typical 4-label κ).
...
3. **Vocabulary index updates needed (queued for next vocab batch):**
   - Add **HMR (Hybrid MR Truth Label)** — canonical definition + cardinality notation
...
4. **Per-domain re-audit candidate:** Pass-65 inconceivability-criterion sweep of urb_608/urb_639/DGI-4/NAD-1 found 0/36 principle reassignments. The HMR audit is structurally DIFFERENT — it asks "does this proposition's CANONICAL CHARACTERIZATION require a single label or a hybrid?" not "does the inconceivability criterion apply?". Audit queued for Pass-71+ at the urb_608 + urb_639 illustrative-examples level.
...
- **Falsifiers initial-inspection-status:** F1 NOT REFUTED on inspection; F2 NOT REFUTED on inspection (5 candidates; need 3-rater verification); F3 OPEN (needs LLM-rater rebuild); F4 OPEN (needs explicit R-HMR seed construction at depth k=15); F5 NOT REFUTED on inspection
- **Biographical anchor:** Brandon's ketamine cool-state catalyzed HMR-3.3 (per #69; per SRC-1-F-3 lived-anchor convention from Pass-66 §7.7.131)
- **Cluster contribution:** +1 (this paper)
