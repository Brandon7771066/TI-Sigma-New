...
The "by-definition" version predicts **perfectly by construction** and is worthless as evidence (it has merely renamed the outcome). GIT-1 uses **only** the prospective score. The whole gap between 1.000 and 0.862 is the difference between **a tautology and a theory**.
...
> * **Anti-circularity clause (the load-bearing one):** GILE-intelligence must be scored **independently of the truth outcome**. Defining it *as* "truth-orientation" makes it unfalsifiable (No-True-Scotsman) and is rejected. GIT-1 is the empirical reconstruction, not the tautology.
...
* **GIT-1-F2 (anti-circularity):** if the GILE→truth signal **survives only** when scoring is allowed to peek at the outcome (i.e., outcome-blind scoring kills it), GIT-1 collapses to a tautology and fails.
