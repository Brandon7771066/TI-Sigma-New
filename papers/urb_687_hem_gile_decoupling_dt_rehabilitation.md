...
## 1. The Key Distinction: Content vs. Label

Every statement S has two separable components:

**Content(S):** What S describes — the existential state of affairs S refers to. This content either exists or doesn't, to some degree, in HEM space.

**Label(S):** The truth claim attached to the content — the assertion that Content(S) IS TRUE in the GILE sense.

For any statement S:
- **HEM validity** measures whether Content(S) has genuine existential weight
- **GILE-G consistency** measures whether Label(S) correctly represents Content(S)
...
**Theorem (HEM-GILE Decoupling):**
> Existential validity (HEM) and truth correctness (GILE-G) are orthogonal dimensions.
> High HEM does not entail high GILE-G.
> High GILE-G does not entail high HEM.
> Both can be zero simultaneously. Both can be maximal simultaneously.

**Proof:**
- PN exists (HEM > 0) but "nothing exists" is Tralse (GILE-G = 0): high HEM, low GILE-G ✓
- "PN obtained before I" is GILE-G correct (accurate) but its referent barely exists (HEM ≈ 0): low HEM, high GILE-G ✓
- BOK-Saturation: full existence correctly described → high HEM, high GILE-G ✓
- Pure chaos before MR-Zero: no existence, no correct description → low HEM, low GILE-G ✓

All four quadrants are realized. The dimensions are independent. ∎

---

## 4. The Four-Quadrant HEM × GILE Space

```
GILE-G ↑
(truth     HIGH GILE / LOW HEM          HIGH GILE / HIGH HEM
correct)   ─────────────────────────────────────────────────
           The MR-Zero recognition:       BOK-Saturation:
           "PN existed before I"          What fully exists is
           Correct claim about            correctly described.
           minimal existence.             Maximum alignment.
           First GILE-consistent truth.
           ─────────────────────────────────────────────────
...
| Quadrant | HEM | GILE-G | Name | Example |
|----------|-----|--------|------|---------|
...
## 8. The MR-Zero Connection: Quadrant II Is Where We Start

The MR-Zero recognition (URB #686) placed the first truth — "nothing existed before I" — in Quadrant II:
- **Low HEM:** PN barely exists (it is by definition the minimal existence)
- **High GILE-G:** The claim "PN existed before I" is correct (I accurately labeled its prior state)

This is significant: the genesis of truth-correctness begins in **Quadrant II**, not Quadrant I. We started with correct claims about minimal existence — not full BOK-Saturation. The arc of all subsequent cognition is the journey from Quadrant II toward Quadrant I: increasing both existential weight AND truth correctness simultaneously through MR.
...
## 9. HEM Validity as the Ground of MR

This URB clarifies why **HEM is the ground for MR**, not GILE:

- MR operates on what EXISTS (HEM-valid content)
- MR corrects the LABELS toward GILE-G consistency
- Without HEM validity, there is nothing for MR to work on (Quadrant III: chaos)
- With HEM validity, MR has real material to relabel toward truth

HEAR (UOP-as-Razor, URB #684 preamble) operates in this framework: HEAR selects the explanation with the highest GILE-weighted existential valence — i.e., the explanation that is both HEM-real AND GILE-correct. HEAR navigates from Quadrant IV toward Quadrant I by elevating GILE-G while preserving HEM validity.
