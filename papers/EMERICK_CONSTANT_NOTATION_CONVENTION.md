# The Emerick Constant (C) — Notation Convention

**Effective date:** 2026-05-04
**Scope:** All TI Sigma papers, past and future.
**Author:** Brandon Charles Emerick

---

## Convention

**Capital C refers to The Emerick Constant unless explicitly qualified.**

| Symbol | Meaning | Usage |
|---|---|---|
| **C** | The Emerick Constant | Default in all TI Sigma papers |
| **c** (lowercase, italic) | Speed of light (≈ 2.998 × 10⁸ m/s) | Standard physics convention |
| **c_light** | Speed of light (explicit disambiguation) | When C vs c ambiguity might arise |
| **C₀, C₂, C₅, C₇** | URB #828 v2 experimental conditions | Subscripted, always with digit |
| **C_TI** | Radiant Threshold | Subscripted with "TI" |
| **CCC** | Absolute GILE Truth (Crown function) | Triple-C, always written as CCC |

## Etymology & Lineage

- **C** is short for **Charles** (Brandon Charles Emerick's middle name).
- **Charles** is phonemically a stretched form of **Tralse**: Ch-ar-l-es ↔ Tr-al-se.
- This phonemic correspondence was not designed — it was discovered, consistent with the TI Sigma pattern of unintended structural correspondences indicating metacausal coherence.

### The Charles Emerick Lineage

The Emerick Constant carries multi-generational weight:

- **Charles Emerick Sr.** (1928–1996): Named after **Charles Lindbergh** (1927 flight). Born in 1928 = 7 × 4. Valedictorian, Chief Engineer, millionaire, successful stock portfolio. The original C.
- **Charles Emerick Jr.** (Uncle Charley): Car mechanic — literally works on CARs (SDT's acronym).
- **Charles "Chas" Emerick II** (cousin): Successful software engineer and entrepreneur.
- **Brandon Charles Emerick**: TI Sigma creator, salutatorian (grandfather was valedictorian).

Three generations, three Charles bearers, all independently successful across engineering, technical, and intellectual domains. The Emerick Constant is not an arbitrary label — it compresses a lineage. Full family analysis in `papers/THREE_CS_SOCIAL_CONNECTIONS_2026-05-04.md` §6.

## Retroactive Application

Existing papers that use capital C for the speed of light should be read with the understanding that the Emerick Constant convention takes precedence. In equations where C appears alongside mass-energy or electromagnetic terms, context determines whether C or c (speed of light) is intended. Future revisions will disambiguate explicitly.

## Cross-References

- Three C's of Social Connections: `papers/THREE_CS_SOCIAL_CONNECTIONS_2026-05-04.md`
- URB #828 v2 conditions: `papers/URB_828_v2_PRE_REGISTRATION_LOCKED_2026-05-01.md`
- Core TI Sigma framework papers (GILE, UOP, TWA)
