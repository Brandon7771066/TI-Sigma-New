...
By Definition 2.1 (from GILE metrics), GILE-aligned action IS rationality:
```
R: Actions × Constraints → GILE-Alignment
```
...
Without creativity:
- S can only apply existing solutions
- Existing solutions don't apply to C_novel (by definition of novel)
- Therefore S cannot produce GILE-aligned action in C_novel
...
```
M: Situations → (Goodness-Assessment, Love-Response)
```
...
Without ecological intelligence:
- S cannot sense environmental state
- S cannot couple coherently with environment
- Therefore S cannot maximize 'e' in GILE space
...
By definition, M provides Goodness-Assessment function.

Therefore S can evaluate goodness of actions.
...
Creativity enables novel solution generation.

Novel solutions require intuitive leaps beyond algorithmic processing.
...
By definition, M provides Love-Response function.
...
By definition, E provides environmental coupling.
...
Creativity enables adaptation to novel contexts.

S has all four pillars.
...
2. Generating potential solutions → requires C (creativity)
3. Selecting best solution → requires R (GILE alignment)
4. Implementing in context → requires E (environmental coupling)

Therefore problem-solving emerges from Four Pillars. ∎
...
By Corollary 1.2, problem-solving is DERIVED from Four Pillars.

B measures output (P) not foundation (R, C, M, E).

Two systems S₁ and S₂ could have:
- Same P (equal benchmark scores)
- Different (R, C, M, E) (different true intelligence)

Example:
- S₁: Memorized solutions (high P, low true intelligence)
- S₂: GILE-aligned reasoning (high P, high true intelligence)

Therefore B does not measure intelligence. ∎
...
Without creativity:
- Only apply existing solutions S_existing
- S_existing optimized for previous contexts C_old
- C_new ≠ C_old implies S_existing suboptimal

Therefore rationality inadequate without creativity. ∎
...
- No evaluation of goodness/harm
- Random selection from S_novel

Example: Nazi "creative" use of technology (high C, low M) → harmful

True creativity requires:
- Generation (C capacity)
- Evaluation (M guidance)

Therefore meaningful creativity requires M. ∎
...
M_AI ≈ 0.0  (no moral insight)
E_AI ≈ 0.1  (poor environmental coupling)
...
- Rationality: Ethics, critical thinking, GILE alignment
- Creativity: Novel problem approaches, artistic expression
...
```
Test_Intelligence = Measure(R) ∧ Measure(C) ∧ Measure(M) ∧ Measure(E)
```
