# Five-Valued Truth: Tralse–Indeterminate Distinction and the Five-Position TI Sigma Logic System

**URB #528 | TI Sigma Framework**
*Brandon M. Emerick — Universal Reality Blueprint Series*
...
## 1. The Problem: Prior Conflation of Tralse and Indeterminate

In TI Sigma's original 4-valued formulations, TRALSE was used to encode both:
1. **Genuine ternary ambiguity** — a value sitting coherently in the middle between True and False
2. **Imperfection/contradiction quality** — the property of having some internal contradiction or incompleteness

This conflation created a theoretical inconsistency: if Tralse can mean both "middle of the spectrum" and "has contradictions," then a paper with LCC = 0.51 (genuinely balanced) looks the same as a paper with internal contradictions that cannot be resolved — which is deeply wrong.

The natural resolution came from recognizing that **Ternary logic already has a dedicated middle slot: Indeterminate.** Tralse is something else.

---

## 2. The Five-Valued System: Formal Definitions

### 2.1 The Three Positional Slots (Ternary Preserved)

Ternary logic has exactly three positions on the truth polarity:

| Value | Integer | Description |
|-------|---------|-------------|
| **True** | 2 | Definitively true, coherent, figured |
| **Indeterminate** | 1 | The genuine middle — 50/50 balanced, coherent irreconcilability |
| **False** | 0 | Definitively false, background, absent |

These are the three *locations* on the truth spectrum. Ternary logic is fully intact.

**Indeterminate** is NOT ambiguity from ignorance — it is *coherent* 50/50 balance. The system knows it is balanced. This is the "45-degree door" of MR2: the door is equally open and closed, and it *knows* it is. This irreconcilability is **coherent**.

### 2.2 The Two Quality Designations (Not Positional Slots)
...
Tralse cannot be "the third value" in ternary logic because it specifies *no location* on the truth polarity. A Tralseness of 0.7 might appear in a True statement (true but imperfect), in a False statement (false but with some grains of truth), or in an Indeterminate statement (balanced but with irreconcilable tensions). Tralse is the *property*, not the *position*.
...
## 3. The Embedding Principle: Tralse Inside the Three

The key insight: **tralseness is embedded in True, False, and Indeterminate.** Every real-world instantiation of any truth value carries some degree of tralse quality. Pure, zero-tralse True would be a Platonic ideal — a mathematical proof of perfect symmetry. Pure, zero-tralse False would be absolute void. Pure, zero-tralse Indeterminate would be a perfect quantum superposition.

In practice, every True is a little Tralse. Every False is a little Tralse. Every Indeterminate is a little Tralse. This is not a weakness — it is the dynamism that allows the system to evolve, improve, and refine.

> "Tralse is the grease that makes the gears run."
...
**LCC range:** [0.8647, 0.9323) — Good and Indeterminate PD zones.

### MR Radiant (GILE Gate)
LCC ≥ 0.9323. The claim is in the Good or Great PD zone. Full causal weight. Tralse quality is present but minimal (the grease keeps working). GILE values are aligned.

---

## 5. Implications for the PD Zone Structure

The PD zones classify LCC scores, not directly truth values. However, the correspondence is:

| PD Zone | LCC Range | MR Status | Truth Quality |
|---------|-----------|-----------|---------------|
| Great | ≥ 0.9323 | MR Radiant | True with minimal Tralse |
...
## 6. Implications for Neural Networks and AGI

Standard neural networks are fundamentally binary at the output layer (softmax = probability distribution over discrete classes, typically treating all uncertainty the same). The 5-valued system proposes three upgrades:

**Upgrade 1: Indeterminate Holding Layer**
Rather than forcing every ambiguous input to the highest-probability class (False clarity), maintain INDETERMINATE states through the network until global context resolves them. This is analogous to quantum superposition: don't collapse prematurely.
...
## 7. Multiple i-Cell Conflict Resolution

A larger i-cell mind (e.g., the cortex) contains many sub-i-cells with potentially conflicting Myrion Resolutions. When:
- Sub-cell A says TRUE
- Sub-cell B says FALSE about the same claim
...
## 8. Summary: Five Values, Three Positions, Two Special Protocols

| Name | Type | Action |
|------|------|--------|
| TRUE | Positional slot | Store, process normally |
...
*Universal Reality Blueprint #528 | Tralse Informationalism*
