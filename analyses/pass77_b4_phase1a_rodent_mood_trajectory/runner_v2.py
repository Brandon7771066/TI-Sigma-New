...
"""
import json, os, sys, time, traceback, warnings, math
...
def main():
    ...
    try:
        ...
        for w in range(n_windows):
            ...
            jval, Gval, Hval = compute_J_window(seg, fs, pairs)
            Mr.append(jval); L_arr.append(Gval); E_arr.append(Hval)
        ...
        results["stages"]["s3_J"] = {
            ...
            "J_mean": float(np.mean(Mr)), "J_std": float(np.std(Mr)),
            "J_min": float(np.min(Mr)), "J_max": float(np.max(Mr)),
            "G_mean": float(np.mean(L_arr)), "G_std": float(np.std(L_arr)),
            "G_max": float(np.max(L_arr)), "n_G_above_G_STAR": n_G_at_cap,
            "H_mean": float(np.mean(E_arr)), "H_std": float(np.std(E_arr)),
        }
        log("   J mean=" + str(round(float(np.mean(Mr)),5)) + " G_mean=" + str(round(float(np.mean(L_arr)),4)) + " G_max=" + str(round(float(np.max(L_arr)),4)) + " n_above_cap=" + str(n_G_at_cap) + " H_mean=" + str(round(float(np.mean(E_arr)),4)))
    except Exception as e:
        log(f"   FAIL: {traceback.format_exc()}")
