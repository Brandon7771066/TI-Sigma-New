"""Experiment A (HEMODYNAMIC) — does the Consciousness-Hamiltonian feature block
improve UNSUPERVISED state decoding over the matched baseline, on fMRI-BOLD and
fNIRS (simulated) + a live rodent neurovascular anchor?

Identical leakage discipline to the LFP batch:
  - latent H leakage-safe (sim: ground truth; live: TRAIN-ONLY k-means on disjoint
    group A, test labeled by nearest train centroid);
...
Feature sets (same readout): BASE | GILEHEM (8-D) | CH (full) | BASE+CH.
"""
import json
import os
import sys
...
def run_source(src):
    ...
    for name, X in [("GILEHEM", GILEHEM), ("CH", CH), ("BASE+CH", BASECH)]:
        row, _ = eval_featureset(X, H, tr, te, K, base_pred=base_pred)
        sets[name] = row
    ...
    return {
        ...
        "fs": float(fs), "n_obs_channels": len(src["groupB"]),
...
def main():
    sources = [
        simulate_bold(seed=0), simulate_bold(seed=7),
        simulate_fnirs(seed=0), simulate_fnirs(seed=7),
    ]
    live_ok = False
    if os.environ.get("HEMO_LIVE", "0") == "1":
        try:
            ...
            if live:
                ...
                print("[runner_hemo] no live rodent hemodynamic source retrieved "
                      "-> sim-only for fMRI/fNIRS (HONESTLY RECORDED).")
        except Exception as e:
            print(f"[runner_hemo] live load error: {type(e).__name__}: {e}")
    else:
        print("[runner_hemo] HEMO_LIVE!=1 -> sim-only run (set HEMO_LIVE=1 to attempt "
              "DANDI neurovascular streaming).")

    out = []
    for src in sources:
        print(f"\n=== {src['label']} ({src['source']} | {src.get('modality','?')}) ===")
        r = run_source(src)
        out.append(r)
    ...
    print(f"\n[runner_hemo] wrote {path} (live_retrieved={live_ok})")
    return out
