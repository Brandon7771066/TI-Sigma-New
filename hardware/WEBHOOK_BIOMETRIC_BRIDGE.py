#!/usr/bin/env python3
"""
Webhook Biometric Bridge
========================
Sends biometric data to webhook.site, which Replit can then fetch.
This bypasses network restrictions that block direct Replit connections.

Usage:
  python WEBHOOK_BIOMETRIC_BRIDGE.py --webhook https://webhook.site/YOUR-ID --mode demo
  python WEBHOOK_BIOMETRIC_BRIDGE.py --webhook https://webhook.site/YOUR-ID --mode mind_monitor
  python WEBHOOK_BIOMETRIC_BRIDGE.py --webhook https://webhook.site/YOUR-ID --mode polar
  python WEBHOOK_BIOMETRIC_BRIDGE.py --webhook https://webhook.site/YOUR-ID --mode all
"""
...
class MindMonitorBridge:
    """Receive OSC from Mind Monitor and forward to webhook"""
    
    def __init__(self, webhook_url, osc_port=5000):
    ...
    def stream(self):
        ...
        print(f"\n{'='*60}")
        print("MIND MONITOR -> WEBHOOK BRIDGE")
        print(f"{'='*60}")
        print(f"Forwarding to: {self.webhook_url}")
            ...
            try:
                ...
                try:
                    ...
                    if resp.status_code in [200, 201]:
                        print(f"[Muse] Alpha={alpha:.2f} -> Webhook OK")
                    else:
...
class PolarH10Bridge:
    """Connect to Polar H10 via Bluetooth and forward to webhook"""
        ...
        try:
            ...
            async def run():
                async with BleakClient(address) as client:
                    print(f"\n{'='*60}")
                    print("POLAR H10 -> WEBHOOK BRIDGE")
                    print(f"{'='*60}")
                    ...
                    def callback(sender, data_bytes):
                        ...
                        try:
                            ...
                            rmssd = hrv.get('rmssd', 0) if hrv else 0
                            if resp.status_code in [200, 201]:
                                print(f"[Polar] HR={hr} RMSSD={rmssd:.1f} -> Webhook OK")
                        except:
                            pass
...
def demo_mode(webhook_url):
    """Send demo data to webhook"""
    print(f"\n{'='*60}")
    print("DEMO MODE -> WEBHOOK")
    print(f"{'='*60}")
    print(f"Target: {webhook_url}")
    print(f"Session: {SESSION_ID}")
    print("\nSending synthetic biometric data...")
    ...
    while True:
        try:
            ...
            if resp.status_code in [200, 201]:
                print(f"[Demo] HR={hr} Alpha={alpha:.2f} -> Webhook OK")
            else:
...
def main():
    parser = argparse.ArgumentParser(description="Webhook Biometric Bridge")
    parser.add_argument('--webhook', required=True, help='Your webhook.site URL')
    parser.add_argument('--mode', choices=['demo', 'mind_monitor', 'polar', 'all'], 
                        default='demo', help='Mode')
    parser.add_argument('--osc-port', type=int, default=5000, help='OSC port')
    
    args = parser.parse_args()
    
    print(f"\n{'='*60}")
    print("WEBHOOK BIOMETRIC BRIDGE")
    print(f"{'='*60}")
    ...
    try:
        if args.mode == 'demo':
            demo_mode(args.webhook)
            
        elif args.mode == 'mind_monitor':
            bridge = MindMonitorBridge(args.webhook, args.osc_port)
            bridge.stream()
            
        elif args.mode == 'polar':
            bridge = PolarH10Bridge(args.webhook)
            bridge.stream()
            
        elif args.mode == 'all':
            print("Starting both bridges...")
            
            muse = MindMonitorBridge(args.webhook, args.osc_port)
            polar = PolarH10Bridge(args.webhook)
