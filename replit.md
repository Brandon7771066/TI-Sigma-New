...
- **UBKI Numerical Findings (URB #786, pilot for close-out path #3)**: `papers/URB_786_UBKI_NUMERICAL_FINDINGS.md` + `riemann_ubki_numerical.py` + `riemann_ubki_report.json` + `riemann_ubki_comparison.png` + `riemann_ubki_spacings.png` + `riemann_zeros_cache.json`. Discretised -i(∂_u + ½) on log-grid u ∈ [-30, 30], GRID=1500, ε=5e-4, and diagonalised five candidate self-adjoint structures (bare periodic Ĥ_∗; +cosh; +|u|; +u²; antiperiodic+cosh). Compared against first N=200 imaginary parts of Riemann zeros from mpmath (cached). **Pilot negative result**: rescaled RMSE 3.40–4.06 across all variants over first 30 zeros (mean rel error 7.25–8.93 %); proper two-sample KS via scipy.stats.ks_2samp on 199 unfolded spacings gives D = 0.503 with p ≈ 3.3e-23 uniformly across A–E (spacing distributions differ from Riemann at extreme significance). Smooth elementary V(u) cannot inject prime-power Selberg-trace structure. **Pilot only — not a full close-out**: original v3 §7.4 specification of "first ~10⁵ zeros" not reached (current dense-diag harness is comfortable up to GRID ≈ 5000); proper exhaustive close-out would need sparse eigensolvers (ARPACK/LOBPCG), parameter sweeps in (GRID, L, EPS), and non-elementary V candidates (e.g. V(u) = Σ_p Σ_k log(p) δ(u−k log p) smoothed). Strong directional signal that effort should redirect to paths #1 (rigorous distributional trace identity) and #2 (Connes adelic identification). v3 conditional theorem UBKI ⟹ RH unchanged; UBKI itself still open. Script's `bk_operator(...)` builder reusable for future UBKI candidates. Millennium Prize **NOT** claimed.
...
- **Axiom Reduction for the UOP Gap in the Riemann Proof (URB #785)**: Reduces the number of irreducibly-TI axioms in the Riemann proof by providing ZFC definitions and reclassifying UOP as a theorem.
- **GILE-HEM Ratio Modulation of PD Expression (URB #784)**: Defines ρ := GILE/HEM as the chirality-breaking parameter of the BOK 4+4 architecture, partitioning ρ-space into four regimes and stating the Beauty Razor Inversion Theorem.
