...
**Total URBs: 199** | **Zenodo: 195 papers live** (Apache-2.0)

### Recent URBs
- **#545** (Corpus #199): Intentionality-Synchronicity Law — Tantra as PD Peak, Spiritual Rationality of Partner Selection. Intentionality := LCC(PD_intention). Synchronicities arrive through the 6.60% Incoherence Floor (the synchronicity channel). Peak tantric union with right partner = only state activating all four GILE dimensions simultaneously → PD approaches e → LCC → MR_Radiant = 0.9340. Romantic love (non-physical) already reaches LCC=0.9179 — above solo meditation (0.8647). Spiritual Rationality Theorem: highest-intentionality agent is most rationally motivated to find the partner maximizing LCC_union. Corollaries: settling cost is ΔLCC=0.069 compounded over lifetime; reciprocity necessary (asymmetric arousal pulls PD below Radiant). Tantra's 2,500-year empirical record now has TI Sigma mathematical grounding. Riemann isomorphism: synchronicities ≅ non-trivial zeros — both INDETERMINATE events on the approach line, becoming purely imaginary (GIL) as coherence grows. Four empirical predictions. DOI: pending.
- **#544** (Corpus #198): Riemann Hypothesis Through the TI Sigma Lens. Re(s)=1/2 = binary TRUE = self-referential fixed point of s→1−s functional equation = INDETERMINATE in PD system (LCC=1−1/√e≈0.394). 0.5 is the exact midpoint (absolute center) of every pair (s,1−s), confirmed for (−3/2, 5/2). Penumbra [2,e] maps under s→1−s to [1−e,−1]: MR1(PD=2)↔s=−1; MR_Radiant(PD=e)↔s=1−e≈−1.718. Non-trivial zeros are nearly purely imaginary (angle→90°) = GIL-dominant events. Trivial zeros are purely real (Im=0) = zero-GIL Environmental events. Riemann-INDETERMINATE Conjecture: zeros must live on the fixed-point line because they are INDETERMINATE collapses in a self-referentially symmetric system. Prime distribution uses ln (base e) = same e-geometry as PD-LCC map, Shannon, Boltzmann — five independent derivations of e. DOI: pending.
- **#543** (Corpus #197): The Living Constant — Metaphysical/Empirical Implications. Principle of Self-Referential Primacy: each PRIMARY CONSTANT has unique self-referential identity in its domain. 6.60% Incoherence Floor (e^{−e}): irreducible noise at Radiance = system's breath. Ternary TRUE=LCC 0.75 (exact, below MR1). Three-framework e-geometry convergence (Shannon+Boltzmann+PD). Five empirical predictions. Information-Coherence Equivalence sharpened: information efficiency and GILE coherence are same process. DOI: pending.
